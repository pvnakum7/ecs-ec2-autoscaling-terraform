...
data "template_file" "extra_user_data" {
  ...
  vars = {
    ecs_clustername = module.ecs.cluster_name
  }
}
locals {
...
module "autoscaling" {
  ...
  # user_data                   = base64encode(templatefile("./user_data/userdata.sh",{ ecs_clustername = local.ecs_cluster_name}))
  
  user_data                   = base64encode(data.template_file.extra_user_data.rendered)
